...
class ResNet(nn.Module):
    ...
    def __init__(self, layers: List[int], num_classess=1000, channels=3):
        super(ResNet, self).__init__()
        self.in_channels = 64
        self.bottleneck = Bottleneck
        # The followling layers define stage 1(befor residual blocks)
        self.conv1 = nn.Conv2d(channels, self.in_channels, kernel_size=7,
    ...
    # Construct ResNet-n
def ResNet18(num_classess, channels):
    return ResNet([2, 2, 2, 2], num_classess, channels)

def ResNet50(num_classess):
    return ResNet([3, 4, 6, 3], num_classess, channels)

def ResNet101(num_classess):
    return ResNet([3, 4, 23, 3], num_classess, channels)

def ResNet152(num_classess):
    return ResNet([3, 8, 36, 3], num_classess, channels)
